 ...
 *
 * pptx-preview 로 슬라이드를 통째로 "그리는" 건 신뢰할 수 없다고 확인됐지만
 * (SlideViewer.tsx 주석 참고), 노트는 이야기가 다르다 — pptx는
 ...
 * 추출"을 할 수 있어야 해서다.
 *
 * CHICODE 에서 그대로 가져왔다. 여기는 해석의 여지가 없는 XML 파싱이라
 * 손댈 이유가 없다.
 */
export async function extractNotesFromPptx(file: Blob): Promise<string[]> {
  // jszip 은 이 함수를 실제로 부를 때만 내려받는다 — PPT 를 올리는 교사만
  // 필요하고, 학생이나 수업 내용을 읽기만 하는 사람은 지지 않아야 할 무게다.
...
async function readXml(zip: JSZipType, path: string): Promise<Document | null> {
