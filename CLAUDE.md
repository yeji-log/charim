...
**1~8단계 완료**(골격 / 로그인·멤버 / 시간표 / 반 명단·수업기록 / 과목·수업자료·핀 게이트 / 수업내용 보드·동아리 / 발표 모드 / 홈). 앱은 `web/` 아래에 있다.
...
**다음은 9단계(정책 문서)** — 마지막이다. 실제 동작을 확인한 뒤 8절 기준으로
쓴다. 텍스트 초안부터 사용자에게 보여줄 것.

홈은 로그인 상태로 두 벌이 갈린다(`Home.tsx` 한 파일 안에서). 교사용은 인사 +
오늘의 수업 + 오늘 기록 여부 + 내 수업 + 공개 주소 복사, 학생용은 히어로 +
선생님 찾기 + 수업자료·동아리 카드다. 학생 카드에서 "일정"은 뺐다 — 눌러봐야
규칙이 읽기를 막아 빈 화면만 본다.
